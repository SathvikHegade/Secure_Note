...
function setupEventListeners() {
  ...
  els.createPassword.addEventListener('input', checkPasswordStrength);
...
async function handleLogin(e) {
  ...
  if (!urlName || urlName.length < 3) {
    if (window.toast) {
      window.toast.error('URL name must be at least 3 characters');
    }
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      if (window.toast) {
        window.toast.success('Access granted! Redirecting...', 2000);
      }
      ...
      if (window.toast) {
        window.toast.error(data.error || 'Incorrect URL or password');
      }
...
function checkPasswordStrength() {
  ...
  let strengthMeter = els.createPassword.parentElement.querySelector('.password-strength');
  if (!strengthMeter) {
    strengthMeter = document.createElement('div');
    strengthMeter.className = 'password-strength';
    strengthMeter.innerHTML = '<div class="password-strength-bar"></div>';
    els.createPassword.parentElement.appendChild(strengthMeter);

    const label = document.createElement('div');
    label.className = 'password-strength-label';
    els.createPassword.parentElement.appendChild(label);
  }

  const bar = strengthMeter.querySelector('.password-strength-bar');
  const label = els.createPassword.parentElement.querySelector('.password-strength-label');
